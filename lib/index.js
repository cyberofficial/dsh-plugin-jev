...
import { registerJevUsageProjection, recordJevUsage, JEV_USAGE_KEY } from './usage.js'
...
export function apply(ctx, config = {}) {
  ...
  // Per-session usage rides the session log as jev/usage events; the projection
  // is what the composer-dock stats chip reads. Register it before the tool so
  // an in-flight call always has a projection to feed.
  ...
  if (config.toolEnabled !== false) {
    registerJevTool(ctx, {
      async run(args, exec) {
        ...
        try {
          recordJevUsage(session, call)
        } catch (error) {
          ctx.logger?.warn?.('dsh-plugin-jev: usage event: ' + error.message)
        }
        try {
          store.record(call)
        } catch (error) {
