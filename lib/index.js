...
import { registerJevUsageProjection, JEV_USAGE_KEY } from './usage.js'
import { createJevService, JEV_SERVICE_NAME } from './service.js'
import { createCommandGuard, defaultGuardConfig, normalizeGuardConfig, SEVERITY_BANDS, DEFAULT_GUARD_TOOLS } from './guard.js'
 ...
 */
export const inject = ['webServer', 'tools', 'systemPrompt', 'sessionProjections']
...
/** Read/write the model preference (GET/POST). */
const SETTINGS_ROUTE_PATH = ROUTE_BASE + '/settings'

/** Read the overall usage aggregate. */
const STATS_ROUTE_PATH = ROUTE_BASE + '/stats'

/** Read/write the tool-command guard config (GET/POST). */
const GUARD_ROUTE_PATH = ROUTE_BASE + '/guard'
  ...
  for (const block of blocks) {
    ...
    switch (block.type) {
      ...
      case 'file':
        ...
        break
      default:
        // reasoning, tool-call and unknown blocks carry no state text here
        break
    }
  }
...
/**
 * Whether a `user/message` carries injected context rather than a human prompt.
 *
 * A user-role message is either the person at the keyboard or content some
 * plugin (or the harness itself) injected on their behalf — file-change
 * notices, AGENTS.md, skill content, goal continuations, cross-session recall,
 * cron notifications. Mixing the two up in the transcript handed to the
 * decision model is exactly the confusion this fold exists to prevent, so the
 * label has to follow the harness's own rule.
 *
 * Session format v4 removed the old catch-all `source.kind === 'plugin'` — the
 * v3→v4 migration rewrites each producer to its own durable kind
 * (`runtime-context`, `agent-instructions`, `skill-invocation`,
 * `session-reference`, `plugin:<name>`, …). `user` is now the single kind
 * reserved for the person, which is the same discriminator the harness's own
 * conversation UI applies (client-ui-chat `message.ts`: an input message is
 * context when `source.kind !== 'user'`). An absent or malformed source is
 * likewise treated as injected: the harness requires one, and guessing
 * "human" for content we cannot attribute is the more damaging error.
 *
 * @param source - the event's `data.source`.
 * @returns true when the message is injected context.
 */
function isInjectedUserMessage(source) {
...
export function foldTranscript(events, options = {}) {
  ...
  for (const event of Array.isArray(events) ? events : []) {
    ...
    if (event.type === 'user/message') {
      line = (isInjectedUserMessage(data.source) ? 'CONTEXT' : 'USER') + ': ' + blocksText(data.content)
      ...
      // Session format v4 flattened tool results: `content` is the result's own
      // blocks (text/image/file) and the failure flag rides the message. The
      // pre-v4 shape nested them under a `tool-result` wrapper block, which the
      // current ContentBlockMap does not define — reading it yields no text and
      // never reports a failure.
      const message = data.message !== null && typeof data.message === 'object' ? data.message : {}
      line = (message.isError === true ? 'TOOL ERROR: ' : 'TOOL RESULT: ') + blocksText(message.content)
    }
...
export async function callTypesafe(options = {}) {
  ...
  const send = async () => {
    ...
    // A caller-owned signal (the command guard, on the dispatch path) aborts the
    // same controller, so cancelling a turn cannot leave the request running.
    // Both reasons land on one controller, so the message distinguishes them by
    // which one is already aborted when the fetch rejects.
    const callerAborted = () => signal !== undefined && signal.aborted === true
    const onCallerAbort = () => controller.abort()
    if (callerAborted()) controller.abort()
    else if (signal !== undefined && typeof signal.addEventListener === 'function') signal.addEventListener('abort', onCallerAbort, { once: true })
    try {
      ...
      clearTimeout(timer)
      if (signal !== undefined && typeof signal.removeEventListener === 'function') signal.removeEventListener('abort', onCallerAbort)
    }
  }
...
/**
 * Merge a posted `commandBlocks` array into the current rules.
 *
 * An entry carrying an `id` updates that rule in place; an entry without one is
 * a new rule and gets a generated id. Returning the whole list is the shape the
 * Settings panel posts, so this also preserves any rule the panel did not send
 * only when the caller omitted the key entirely (handled by the route).
 *
 * @param current - the existing normalized rules.
 * @param incoming - the posted array, or a non-array to leave rules untouched.
 * @returns the merged rule list, or the current list when nothing usable came in.
 */
export function mergeCommandBlocks(current, incoming) {
  ...
  for (const entry of incoming) {
    ...
    if (id === '') {
      // Stable, human-readable, and unique within this list. Never derived from
      // user text, so a crafted pattern cannot collide with an existing id.
      do {
        generated += 1
        id = 'rule-' + generated
      } while (used.has(id) || existing.some((rule) => rule.id === id))
    }
    used.add(id)
...
/**
 * The `/guard` response body, shared by GET and POST so the two can never
 * disagree about their shape.
 * @param guard - the command guard.
 * @param store - the plugin store (unused today, kept for symmetry with peers).
 * @returns the guard payload the Settings panels read.
 */
export function guardPayload(guard, store) {
  return {
    config: guard.getConfig(),
    stats: guard.stats(),
    // The block log: what the guard actually refused, newest first, with the
    // command and the deciding severity/rule. In memory only, so it is a "what
    // just happened" view rather than a durable audit trail.
    denials: typeof guard.denials === 'function' ? guard.denials() : [],
    defaults: {
      blockThreshold: defaultGuardConfig().blockThreshold,
      confidenceFloor: defaultGuardConfig().confidenceFloor,
      tools: DEFAULT_GUARD_TOOLS.slice(),
      severityBands: SEVERITY_BANDS.slice(),
    },
  }
}
...
export function apply(ctx, config = {}) {
  ...
  const store = config.store instanceof JevStore
    ? config.store
    : typeof config.statePath === 'string'
      ? new JevStore(config.statePath)
      : JevStore.atHome()

  // Per-session usage rides the session log via the tool's tool/result meta;
  // the jevUsage projection folds those events for the composer-dock stats chip.
  // Register it before the tool so an in-flight call always has a projection to
  // feed.
  registerJevUsageProjection(ctx)

  // The host-facing surface: the same validated call, exposed to sibling
  // plugins as an injectable service so they can get a calibrated judgment
  // without spending a main-model round trip. Host calls are recorded in the
  // aggregate with `source: 'host'`; they cannot reach the per-chat chip, whose
  // only source is session-log tool/result meta.
  const jevService = createJevService(ctx, {
    baseURL,
    fetchImpl,
    timeoutMs,
    costPerToken,
    defaultModel: store.modelOr(DEFAULT_MODEL),
    onUsage(call) {
      store.record(call)
    },
  })
  ctx.provide(JEV_SERVICE_NAME, jevService)

  // The tool-command guard: it can deny a shell command before dispatch, from
  // either user-authored blocked-command rules or Jev's own danger score. It
  // rides the same service, so a scored command is one upstream call and lands
  // in the aggregate under `source: 'guard'`.
  const guard = createCommandGuard(ctx, {
    config: store.state.guard,
    ask: (input) => jevService.ask({ ...input, source: 'guard' }),
    // The guard's counters are all-time numbers: seeded from what the store
    // persisted last run, and handed back after every command it looks at, so
    // the Settings figures survive a harness restart.
    initialCounters: store.state.guardCounters,
    onCounters: (counters) => { store.setGuardCounters(counters) },
  })
  const guardConfig = () => guard.getConfig()
  ctx.effect(
    () => ctx.on('tools/pre-execute', guard.handler),
    'dsh-plugin-jev: tool command guard',
  )
  ...
  if (config.toolEnabled !== false) {
    registerJevTool(ctx, {
      async run(args, exec) {
        const request = buildRequest({ state: args.state, questions: args.questions, model: args.model || store.modelOr(DEFAULT_MODEL) })
        ...
        const sessionId = session !== null && session !== undefined && typeof session.id === 'string' ? session.id : null
        // Per-session usage is projected onto this call's tool/result meta by
        // output.presentationMeta (see tool.js); only the cross-session
        // aggregate needs the store.
        const call = {
          model: result.model,
        ...
        try {
          ...
          ctx.logger?.warn?.('dsh-plugin-jev: usage store: ' + error.message)
        }
        return {
          model: result.model,
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        // The route goes through the host service, not straight to askJev: the
        // service records the call in the aggregate under `source: 'host'`,
        // which a direct askJev call never did — HTTP asks worked but their
        // spend was invisible in the totals. The model fallback stays a LIVE
        // read (the service's default is captured once at mount), matching what
        // the tool path honours after a settings change.
        const result = await jevService.ask({
          state: body !== null && typeof body === 'object' ? body.state : undefined,
          questions: body !== null && typeof body === 'object' ? body.questions : undefined,
          model: body !== null && typeof body === 'object' && body.model ? body.model : store.modelOr(DEFAULT_MODEL),
        })
        return sendJson(res, 200, result)
      } catch (error) {
        ctx.logger?.warn?.('dsh-plugin-jev: ask: ' + error.message)
        const status = error && error.code === 'no-key' ? 401
          : error && error.status === 429 ? 429
          // The service's own seam validation means the CALLER is malformed,
          // which is a 400; anything else is our failure or the upstream's.
          : error && error.code === 'bad-input' ? 400
          : 502
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        if (method === 'POST') {
          const body = await readJsonBody(req)
          const model = body !== null && typeof body === 'object' && typeof body.model === 'string' ? body.model.trim() : ''
          if (model === '') return sendJson(res, 400, { error: 'a non-empty model name is required' })
          store.setModel(model)
        }
        return sendJson(res, 200, {
          model: store.modelOr(DEFAULT_MODEL),
          storedModel: store.state.model,
          defaultModel: DEFAULT_MODEL,
          updatedAt: store.state.updatedAt,
        })
      } catch (error) {
        ctx.logger?.warn?.('dsh-plugin-jev: settings: ' + error.message)
        return sendJson(res, 502, { error: error.message })
      }
    },
  }), 'dsh-plugin-jev: settings route')

  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: STATS_ROUTE_PATH,
    handler: (req, res) => {
      return sendJson(res, 200, Object.assign(store.snapshot(), {
        defaultModel: store.modelOr(DEFAULT_MODEL),
        costPerToken,
        projectionKey: JEV_USAGE_KEY,
        guard: guard.stats(),
      }))
    },
  }), 'dsh-plugin-jev: stats route')

  ctx.effect(() => ctx.webServer.register({
    kind: 'exact',
    path: GUARD_ROUTE_PATH,
    handler: async (req, res) => {
      const method = String(req.method || 'GET').toUpperCase()
      try {
        if (method === 'POST') {
          ...
          if (body === null || typeof body !== 'object' || Array.isArray(body)) {
            return sendJson(res, 400, { error: 'a JSON object patch is required' })
          }
          const current = guard.getConfig()
          // A partial patch: only the keys actually present are replaced, so the
          // Settings panels can save one control without echoing the whole config.
          const patch = {}
          for (const key of Object.keys(body)) {
            if (key === 'commandBlocks') {
              patch.commandBlocks = mergeCommandBlocks(current.commandBlocks, body.commandBlocks)
              continue
            }
            patch[key] = body[key]
          }
          const next = normalizeGuardConfig({ ...current, ...patch })
          guard.setConfig(next)
          store.setGuard(next)
        }
        return sendJson(res, 200, guardPayload(guard, store))
      } catch (error) {
        ctx.logger?.warn?.('dsh-plugin-jev: guard route: ' + error.message)
        return sendJson(res, 400, { error: error.message })
      }
    },
  }), 'dsh-plugin-jev: guard route')
}
