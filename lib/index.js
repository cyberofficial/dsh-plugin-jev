 ...
 *     Settings tab and the composer-dock pill (the literal never leaves here);
 *   - a session-to-text transcript fold so the browser can offer "current chat"
 *     as the state to evaluate.
 *
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const key = await resolveApiKey(ctx)
        if (key === null) {
          return sendJson(res, 401, { error: 'TypeSafe API key is not set — open Settings, then Plugins, then Jev (TypeSafe), or the Jev pill, to store it.' })
        }
        const payload = await callTypesafe({ apiKey: key.value, baseURL, path: '/systemone', method: 'POST', body: request, fetchImpl, timeoutMs })
        const normalized = normalizeAnswers(payload)
        return sendJson(res, 200, {
          ...normalized,
          costUsd: costOfUsage(normalized.usage, costPerToken),
          elapsedMs: Date.now() - started,
          credential: { configured: true, ref: key.ref, source: key.source },
        })
      } catch (error) {
        ctx.logger?.warn?.('dsh-plugin-jev: ask: ' + error.message)
        const status = error && error.status === 401 ? 401 : error && error.status === 429 ? 429 : 502
