 ...
 * host-side service (aggregate only).
 */
export const USAGE_SOURCES = ['tool', 'host']
...
export function normalizeState(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    return { model: null, totals: emptyUsage(), byModel: {}, bySource: normalizeSourceMap(null), sessions: {}, lastCall: null, updatedAt: null }
  }
  const byModel = {}
...
export class JevStore {
   ...
   * @param {{ sessionId?: string|null, model?: string|null, inputTokens: number, outputTokens: number, costUsd: number, source?: 'tool'|'host' }} call
   *   `source` defaults to `'tool'`, the agent's `jev_ask` dispatch; pass
   *   `'host'` for a sibling plugin calling the host-side service.
