...
/**
 * The guard's lifetime counters, persisted so "how many commands has the guard
 * looked at" survives a harness restart. Keys mirror lib/guard.js's runtime
 * counters; `last` (the panel's last-decision record) rides along because the
 * Settings panel renders it from the same object.
 */
export const GUARD_COUNTER_KEYS = ['evaluated', 'blockedLiteral', 'blockedSemantic', 'blockedDanger', 'blockedOversize', 'downgraded', 'failures', 'cached', 'skipped']

/** Normalize an arbitrary value into the persisted guard-counter shape. */
function normalizeGuardCounters(value) {
  const source = value !== null && typeof value === 'object' && !Array.isArray(value) ? value : {}
  const out = {}
  for (const key of GUARD_COUNTER_KEYS) out[key] = int(source[key])
  out.last = source.last !== null && typeof source.last === 'object' && !Array.isArray(source.last) ? source.last : null
  return out
}
...
export function normalizeState(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    return { model: null, guard: null, guardCounters: normalizeGuardCounters(null), totals: emptyUsage(), byModel: {}, bySource: normalizeSourceMap(null), sessions: {}, lastCall: null, updatedAt: null }
  }
  const byModel = {}
  ...
  return {
    ...
    guardCounters: normalizeGuardCounters(value.guardCounters),
...
export class JevStore {
  ...
  /**
   * Persist the guard's lifetime counters.
   *
   * The guard reports its counters after every command it looked at; a no-op
   * report (unchanged numbers, e.g. the guard skipped the command entirely) is
   * detected by comparison and does not touch the file, so a disabled guard
   * costs no writes at all.
   *
   * @param {object} value - the guard's counters in the persisted shape.
   * @returns {boolean} whether the persisted state changed.
   */
  setGuardCounters(value) {
    const next = normalizeGuardCounters(value)
    let changed = false
    for (const key of GUARD_COUNTER_KEYS) {
      if (next[key] !== this.state.guardCounters[key]) { changed = true; break }
    }
    if (!changed && JSON.stringify(next.last) === JSON.stringify(this.state.guardCounters.last)) return false
    this.state = Object.assign({}, this.state, { guardCounters: next, updatedAt: this.now() })
    this.save()
    return true
  }
  ...
  record(call) {
    ...
    this.state = {
      ...
      // Same for the guard's lifetime counters, which the guard itself updates.
      guardCounters: this.state.guardCounters,
      totals,
      byModel,
      bySource,
      sessions,
  ...
  snapshot() {
    return {
      ...
      // The guard's lifetime counters, deep-copied for the same reason.
      guardCounters: JSON.parse(JSON.stringify(this.state.guardCounters)),
