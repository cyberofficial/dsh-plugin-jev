...
/**
 * A malformed CALLER input: wrong types, missing fields, over budget.
 *
 * Tagged with `.code === 'bad-input'` so a route fronting the service can
 * answer 400 (the caller's mistake) instead of 502 (which would claim our
 * upstream misbehaved). Deeper value problems, like a question shape the API
 * rejects, are thrown by buildRequest and deliberately carry no code.
 */
 ...
 * @throws {@link ServiceInputError} when state or questions are missing or
 *   malformed, or the state is larger than the service budget.
 */
function assertServiceInput(input) {
  if (input === null || typeof input !== 'object' || Array.isArray(input)) {
    throw new ServiceInputError('jev.ask({ state, questions }) requires an options object')
  }
  if (input.state === undefined || input.state === null) {
    throw new ServiceInputError('jev.ask: state is required and must be a non-empty string, object, or array')
  }
  ...
  if (typeof input.state === 'string' ? input.state.trim() === '' : size === 0) {
    throw new ServiceInputError('jev.ask: state must be non-empty')
  }
  if (input.questions === undefined || input.questions === null) {
    throw new ServiceInputError('jev.ask: questions is required and must be a map of typed questions')
  }
  if (typeof input.questions !== 'object' || Array.isArray(input.questions)) {
    throw new ServiceInputError('jev.ask: questions must be a JSON object keyed by question id')
  }
  if (Object.keys(input.questions).length > MAX_SERVICE_QUESTIONS) {
    throw new ServiceInputError('jev.ask: too many questions (max ' + MAX_SERVICE_QUESTIONS + ' per call)')
  }
  if (input.model !== undefined && input.model !== null && typeof input.model !== 'string') {
    throw new ServiceInputError('jev.ask: model must be a string alias or id when present')
  }
}
...
export function createJevService(ctx, options = {}) {
   ...
   * @throws a {@link ServiceInputError} (`.code === 'bad-input'`) on malformed
   *   input, or an `Error` with `.code === 'no-key'` when no TypeSafe credential
   *   is configured. Consumers that gate agent behaviour should catch and fail
   *   open.
   */
