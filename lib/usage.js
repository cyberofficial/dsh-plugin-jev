 ...
 * end (renderJevResult in tool.js). The match is anchored on the first line
 * AND the trailer line, so a tool result that merely echoes a Jev answer
 * inside a larger output cannot count.
 *
...
/**
 * Collect the model-facing text of a tool/result message: the message's
 * content holds one tool-result block whose own content is the render blocks.
 * Returns '' unless the shape is exactly that of a plain text render.
 */
...
export function foldJevUsage(state, event) {
  ...
  if (event.type === 'tool/result') {
    ...
    source = usage !== null ? usage : parseJevRenderText(toolResultText(data.message))
